...
<div class="app">
  <div class="topbar">
    ...
    <div class="who">
      ...
      <a href="duty_assignments.php" class="btn btn-ghost current">Duty Assignments</a>
  ...
  <div class="panel">
    <form method="get" action="duty_assignments.php" id="dutyFilterForm">
      <div class="panel-row">
        <div class="control" style="flex:2;min-width:320px;">
          <label>Search</label>
          <input type="text" id="combinedSearch" class="search-input" placeholder="Search Member, State or District">
          <div id="combinedPanel" class="combo-panel" style="display:none;position:relative;z-index:40;max-height:220px;overflow:auto;margin-top:6px;border:1px solid var(--line);background:#fff;border-radius:6px;"></div>
        </div>

        <!-- Keep the original three filters in the form but hide them from view; they remain for compatibility. -->
        ...
        <div class="control" style="display:none;">
          <label>Member</label>
          <select name="personnel_id" id="personnelFlSelect" class="search-input combo-select" data-placeholder="All Members" onchange="document.getElementById('dutyFilterForm').submit()">
        ...
        <?php endif; ?>
        <a class="btn btn-secondary" href="duty_assignments.php?export=1&<?= buildDutyQuery() ?>" style="align-self:flex-end;padding:10px 16px;">Export PDF</a>
      </div>
    </form>
  </div>
...
<script>
  ...
  var ALL_PERSONNEL = <?= json_encode($allPersonnel) ?>;

  // Combined search (State / District / Member) for the filter panel.
  (function(){
    var input = document.getElementById('combinedSearch');
    var panel = document.getElementById('combinedPanel');
    var form = document.getElementById('dutyFilterForm');
    var stateSel = document.getElementById('stateFlSelect');
    var districtSel = document.getElementById('districtFlSelect');
    var personnelSel = document.getElementById('personnelFlSelect');

    if (!input || !panel || !form) return;

    function populateDistrictsFor(state, selected) {
      // Replace districtSel options based on STATES_DISTRICTS[state]
      if (!districtSel) return;
      districtSel.innerHTML = '';
      var opt = document.createElement('option'); opt.value = ''; opt.textContent = 'All Districts';
      districtSel.appendChild(opt);
      var list = STATES_DISTRICTS[state] || [];
      list.forEach(function(d){
        var o = document.createElement('option'); o.value = d; o.textContent = d; if (selected && selected === d) o.selected = true; districtSel.appendChild(o);
      });
    }
    ...
    function render(items){
      panel.innerHTML = '';
      if (!items.length) { var empty = document.createElement('div'); empty.className = 'combo-empty'; empty.textContent = 'NoSearch'; panel.appendChild(empty); openPanel(); return; }
      items.forEach(function(it){
        var row = document.createElement('div');
        row.className = 'combo-option';
        row.textContent = it.display;
        row.dataset.type = it.type;
        if (it.type === 'state') row.dataset.state = it.state;
        if (it.type === 'district') { row.dataset.state = it.state; row.dataset.district = it.district; }
        if (it.type === 'member') row.dataset.memberId = it.id;
    ...
    function choose(it){
      if (it.type === 'state') {
        if (stateSel) stateSel.value = it.state;
        if (districtSel) populateDistrictsFor(it.state, '');
        if (personnelSel) personnelSel.value = '';
      } else if (it.type === 'district') {
        if (stateSel) stateSel.value = it.state;
        if (districtSel) populateDistrictsFor(it.state, it.district);
        if (personnelSel) personnelSel.value = '';
      } else if (it.type === 'member') {
        if (personnelSel) personnelSel.value = it.id;
        // clear state/district selections when a member is chosen
        if (stateSel) stateSel.value = '';
        if (districtSel) { districtSel.innerHTML = ''; var o = document.createElement('option'); o.value = ''; o.textContent = 'All Districts'; districtSel.appendChild(o); }
      }
    ...
    function buildResults(q){
      ...
      if (q === '') return [];
      var results = [];
      // Members first (if match name)
      for (var i = 0; i < ALL_PERSONNEL.length && results.length < 10; i++) {
        var p = ALL_PERSONNEL[i];
        var txt = (p.name || '').toLowerCase();
        if (txt.indexOf(q) !== -1) {
          results.push({ type: 'member', id: p.id, display: p.name + (p.rank_name ? ' (' + p.rank_name + ')' : '') });
        }
      }
      // States
      Object.keys(STATES_DISTRICTS).forEach(function(st){ if (results.length >= 12) return; if (st.toLowerCase().indexOf(q) !== -1) results.push({ type: 'state', state: st, display: st + ' (State)' }); });
      // Districts (include state for context)
      Object.keys(STATES_DISTRICTS).forEach(function(st){
        var arr = STATES_DISTRICTS[st] || [];
        arr.forEach(function(ds){ if (results.length >= 12) return; if (ds.toLowerCase().indexOf(q) !== -1) results.push({ type: 'district', state: st, district: ds, display: ds + ' — ' + st }); });
      });
    ...
    input.addEventListener('focus', function(){ var q = input.value || ''; var res = buildResults(q); render(res); });
    document.addEventListener('click', function(ev){ if (!panel.contains(ev.target) && ev.target !== input) closePanel(); });
  })();
