...
parsing, POST handling, PDF export, stats and HTML together. They're
...
- **Duty & Operation Location, expanded** (`duty_assignments.php`):
  ...
  - **PDF export (watermarked)** for duty records, honoring whatever
    search/member/rank/status filters are currently applied.
  - Note: Confidential-type duty records are **not** hidden or
    redacted from other admins or from PDF export — every admin sees
...
4. **Ciri baru — Tugasan & Lokasi Operasi** (`duty_assignments.php`): admin
   ...
   **tarikh** dan **tujuan/nama operasi**. Papan pemuka admin memaparkan
   ...
   individu.
5. **Eksport PDF** — dua butang eksport, dan **nama fail sentiasa
   mengandungi tarikh hari ia dieksport**:
   - *Eksport Bulan Ini* → `SOPRA_Bayaran_<Bulan>_<Tahun>_Eksport_<tarikh-hari-ini>.pdf`
     — senarai siapa sudah/belum bayar bulan tersebut, jumlah RM dan
     hari mereka bayar.
   - *Eksport Tahun Penuh* → `SOPRA_Bayaran_<Tahun>_Eksport_<tarikh-hari-ini>.pdf`
...
action/export → `require` view). Logik POST, eksport PDF dan HTML
...
│
├── exports/                 # eksport PDF (watermark logo) (satu fail = satu halaman)
